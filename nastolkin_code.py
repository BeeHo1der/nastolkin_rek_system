...
def get_recommendations_with_profile(message, game_names, user_id, n_recommendations=1):
    ...
    bot.send_message(message.chat.id, f"Вот рекомендация для вас:\nНазвание: {recs[9]}\nХарактеристики:\nКоличество игроков: от {recs[10]} до {recs[11]}.\nИгровое время: от {recs[12]} до {recs[13]} минут.\nВозрастной рейтинг: от {recs[14]} лет.\nГод выхода игры: {recs[15]}.\nСсылка на игру: https://boardgamegeek.com{recs[16]}")
    return recommendations


def description(game_name):
    try:
        s=f'Название: {game_name}\nХарактеристики:\nКоличество игроков: от {df_games.loc[df_games['name'] == game_name, 'minplayers'].values[0]} до {df_games.loc[df_games['name'] == game_name, 'maxplayers'].values[0]}.\nИгровое время: от {df_games.loc[df_games['name'] == game_name, 'minplaytime'].values[0]} до {df_games.loc[df_games['name'] == game_name, 'maxplaytime'].values[0]} минут.\nВозрастной рейтинг: от {df_games.loc[df_games['name'] == game_name, 'playerage'].values[0]} лет.\nГод выхода игры: {df_games.loc[df_games['name'] == game_name, 'yearpublished'].values[0]}.\nСсылка на игру: https://boardgamegeek.com{df_games.loc[df_games['name'] == game_name, 'gamelink'].values[0]}'
        return s
    except:
...
button_1 = telebot.types.KeyboardButton('Добавить любимую игру')
button_2 = telebot.types.KeyboardButton('Добавить непонравившуюся игру')
button_3 = telebot.types.KeyboardButton('Список любимых игр')
button_4 = telebot.types.KeyboardButton('Список непонравившихся игр')
button_5 = telebot.types.KeyboardButton('Убрать любимую игру')
button_6 = telebot.types.KeyboardButton('Убрать непонравившуюся игру')
button_7 = telebot.types.KeyboardButton('Рекомендации')
button_8 = telebot.types.KeyboardButton('Главное меню')
button_9 = telebot.types.KeyboardButton('Узнать больше об определенной игре')
...
def send_welcome(message):
    bot.reply_to(message, "😊 Привет! Я твой гид по миру настольных игр! \n🎲 Выбирай, какую услугу хочешь получить:", reply_markup=keyboard)


@bot.message_handler(func=lambda message: message.text == 'Главное меню')
def send_welcome(message):
    bot.reply_to(message,
                 "😊 Привет! Я твой гид по миру настольных игр! \n🎲 Выбирай, какую услугу хочешь получить:",
                 reply_markup=keyboard)


@bot.message_handler(func=lambda message: message.text == 'Узнать больше об определенной игре')
def descr(message):
...
@bot.message_handler(func=lambda message: message.text == 'Добавить любимую игру')
...
@bot.message_handler(func=lambda message: message.text == 'Добавить непонравившуюся игру')
...
@bot.message_handler(func=lambda message: message.text == 'Список любимых игр')
def handle_show_likes(message):
    user_id = message.from_user.id
    show_liked_games(message, user_id)


@bot.message_handler(func=lambda message: message.text == 'Список непонравившихся игр')
def handle_show_dislikes(message):
    user_id = message.from_user.id
    show_disliked_games(message, user_id)


@bot.message_handler(func=lambda message: message.text == 'Убрать любимую игру')
...
@bot.message_handler(func=lambda message: message.text == 'Убрать непонравившуюся игру')
